...
## 11. Focus Ring / Keyboard Navigation

- **Dùng `focus-visible:ring-*` thay vì `focus:ring-*`** trên tất cả interactive elements (Button, Select, Input, v.v.).
- `focus:ring-*` hiện focus ring cả khi click chuột → gây cảm giác "element vẫn bị giữ focus" sau khi chọn xong.
- `focus-visible:ring-*` chỉ hiện focus ring khi người dùng điều hướng bằng bàn phím (Tab, arrow keys) — đúng hành vi mong đợi cho cả mouse và keyboard user.
- **Áp dụng toàn hệ thống**, bao gồm mọi component trong `components/ui/` (select.tsx, button.tsx, input.tsx, ...).

---

### 11.2 Dropdown / Popover không chặn click vào element bên dưới

Click vào một input trong khi dropdown đang mở phải focus vào input ngay lập tức — không yêu cầu click 2 lần.

Có 2 cơ chế Radix UI cần fix đồng thời trên `<Select>`:

1. **`modal={false}`** trên `SelectPrimitive.Root` — tắt focus trap modal, cho phép tương tác với các element bên ngoài dropdown.
2. **`onCloseAutoFocus={(e) => e.preventDefault()}`** trên `SelectPrimitive.Content` — ngăn Radix tự động trả focus về trigger khi dropdown đóng (đây là nguyên nhân chính gây ra "click lần 2 mới focus được").

Nếu chỉ fix `modal={false}` mà không fix `onCloseAutoFocus`: khi user click vào input, Select đóng lại → Radix trả focus về trigger → input không nhận được focus → user phải click lần 2.

**Đã áp dụng vào `components/ui/select.tsx`** — không cần truyền lại ở từng nơi sử dụng.

Quy tắc tương tự áp dụng cho `Popover`, `DropdownMenu`, `Combobox` và các overlay Radix UI khác có `onCloseAutoFocus`.

---

## 12. Field Display & Input Rules
...
- **Cho phép chọn ngày trong tương lai** — không disable các ngày sau ngày hôm nay (trừ khi field cụ thể có yêu cầu khác).
- **Preset "Tháng này":** set `from = ngày 1 của tháng hiện tại`, `to = ngày cuối của tháng hiện tại` (full tháng, kể cả ngày tương lai trong tháng).
- **Preset "Tuần này":** set `from = thứ Hai của tuần hiện tại`, `to = Chủ Nhật của tuần hiện tại` (full tuần ISO, kể cả ngày tương lai trong tuần).
